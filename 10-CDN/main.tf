resource "aws_cloudfront_distribution" "chandulearn" {
     enabled             = true
  ...
  records = [
    {
      ...
      type    = "A"
      alias   = {
        name    = aws_cloudfront_distribution.chandulearn.domain_name
        zone_id = aws_cloudfront_distribution.chandulearn.hosted_zone_id
      }
    }
  ]
}
